...
function MultiplayerQuizResultsPage() {
	const { roomCode } = useParams({
		from: "/_app/_authenticated/multiplayer/$roomCode/results",
	});
	const navigate = useNavigate();

	const resultsDataQuery = useQuery(
		api.multiplayer.getMultiplayerQuizResultsData,
		{
			roomCode,
		},
	);
	...
	if (resultsDataQuery === undefined || currentUserQuery === undefined) {
		return (
			<div className="flex flex-col items-center justify-center min-h-screen p-4 bg-[var(--background)] text-[var(--foreground)]">
				...
				<Text
					as="h2"
					className="text-2xl font-[var(--font-head)]"
					style={{
	...
	if (!players) {
		return (
			<div className="flex flex-col items-center justify-center min-h-screen p-4 bg-[var(--background)] text-[var(--accent)]">
				...
				<Text as="p" className="mt-2 text-center text-[var(--foreground)]">
					Player data is currently unavailable for this quiz.
				</Text>
				<Button
					onClick={() => navigate({ to: "/dashboard", replace: true })}
					className="mt-6"
		...
		<div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] font-[var(--font-sans)] flex flex-col items-center justify-center p-4">
			<Card className="w-full max-w-2xl shadow-[var(--shadow-xl)] border-2 border-[var(--border)] bg-[var(--card)] text-[var(--card-foreground)] my-8">
				<Card.Header className="text-center">
					...
					<Text
						as="h1"
						className="text-3xl md:text-4xl font-[var(--font-head)]"
						style={{
							textShadow: "0 0 5px var(--primary), 0 0 10px var(--primary)",
						}}
					>
				...
				<Card.Content className="p-6 md:p-8 space-y-4 max-h-[60vh] overflow-y-auto">
					{players.map((player, idx) => (
						...
						>
							<Text
								as="p"
								className={`mr-4 text-xl w-8 text-center text-[var(--foreground)]"}`}
							>
								{idx + 1}
								{player.score === topScore && topScore > 0 && idx === 0 && (
									<Trophy className="inline h-5 w-5 ml-1 text-[var(--primary)]" />
								)}
								{player.score === topScore &&
									topScore > 0 &&
									idx > 0 &&
									player.score === players[idx - 1].score && (
										<Trophy className="inline h-5 w-5 ml-1 text-[var(--primary)]" />
									)}
							</Text>
							...
							<div className="flex-grow">
								<Text
									as="p"
									className={`text-lg truncate text-[var(--foreground)]"}`}
								>
									{player.username}
									{player.userId === hostId && (
										<Crown className="inline h-4 w-4 ml-1.5" />
									)}
								</Text>
								<Text as="p" className={"text-md text-[var(--foreground)]"}>
									Score: {player.score}
								</Text>
							</div>
						</div>
					...
					{players.length === 0 && (
						<Text
							as="p"
							className="text-center text-[var(--muted-foreground)] py-4"
						>
							No players participated or results are not available.
						</Text>
					)}
				</Card.Content>
