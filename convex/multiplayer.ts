...
export const startRoom = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (room.status !== "waiting") {
      throw new Error("Quiz cannot be started. It might be already active or finished.");
    }
...
export const submitAnswer = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.runMutation(api.multiplayer.advanceToNextQuestionOrFinish, { // Use api.multiplayer
      roomId: args.roomId,
    });
...
export const internalHandleQuestionTimeout = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!room || room.status !== "active" || room.currentQuestionIndex !== args.expectedQuestionIndex) {
      console.log(`Timeout for room ${args.roomId}, question ${args.expectedQuestionIndex} is stale. Current index: ${room?.currentQuestionIndex}, status: ${room?.status}`);
      return;
    }

    console.log(`Timeout triggered for room ${args.roomId}, question ${args.expectedQuestionIndex}. Advancing.`);
...
export const getMultiplayerQuizPlayData = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (room.status !== "active" && room.status !== "finished") {
      return { error: "Quiz is not active or has not finished yet.", status: room.status };
    }
    ...
    const currentPlayer = await ctx.db
      .query("multiplayer_players")
      .withIndex("by_room_user", (q) =>
        q.eq("roomId", room._id).eq("userId", user._id) // Use room._id from the fetched room
      )
      .unique();
    ...
    return {
      ...
      allPlayers: await Promise.all(
        (await ctx.db.query("multiplayer_players").withIndex("by_room", q => q.eq("roomId", room._id)).collect())
        .map(async (p) => {
...
export const getMultiplayerQuizResultsData = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (room.status !== "finished" && room.status !== "active") {
      return { error: "Quiz is not yet finished or active.", status: room.status };
    }
